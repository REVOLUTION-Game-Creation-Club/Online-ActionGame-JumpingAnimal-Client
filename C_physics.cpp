...
// 연산이 이루어지는 부분
// impulse 구현이 아직 안됐음
void C_physics:: step( /*DWORD time*/) {
	C_obj* temp;// = new C_obj("test",1,"square");

	float m/*질량*/; //impulse 구현시 사용
	float vx,vy,storex/*속도:이동하는 픽셀*/;
	int tx,ty/*계산될 좌표*/;
	...
	for(i = 0; i<m_list_index; i++){
		...
		if(temp->repitition != 1)
		{
			if(GetTickCount() - temp->FastTime > 5000)
				temp->repitition = 1;
		}
		temp = temp->next;
	}
...
void C_physics::Test()
{
	C_obj* temp;
	int i = 0;
	for(temp = movable_first ; temp ; temp = temp->next) i++;
	if(i != m_list_index )
	{
		int a, b;
		a = 10;
...
void C_physics::movable_obj_exclude( C_obj* exobj )
{
	if(m_list_index == 0)
	{
#ifdef _DEBUG
	...
	else if(movable_first == exobj)
	{
		if(exobj->next == NULL)
			movable_obj_list = NULL;
		movable_first = exobj->next;
		m_list_index--;
#ifdef _DEBUG
		return Test();
#endif
	...
	{
		C_obj* temp;
		for(temp = movable_first ; !(temp->next == exobj || temp->next == NULL) ; temp = temp->next);
		if(temp->next != exobj)
#ifdef _DEBUG
			return Test();
#endif
		...
		if(exobj->next == NULL)
			movable_obj_list = temp;

		temp->next = exobj->next;
		m_list_index--;
	}
#ifdef _DEBUG
	return Test();
...
BOOL C_physics:: culision_check( C_obj* culision_main) {
	...
	{
		//충돌 여부
		if( culision_main->x >= temp->x && culision_main->x <= (temp->x + temp->width) ||
			(culision_main->x + culision_main->width) >= temp->x && (culision_main->x + culision_main->width)  <= (temp->x + temp->width) ){
				if(culision_main->y >= temp->y && culision_main->y <= (temp->y + temp->height) ||
					(culision_main->y + culision_main->height) >= temp->y && (culision_main->y + culision_main->height) <= (temp->y + temp->height))
					...
					(culision_main->y + culision_main->height) >= temp->y && (culision_main->y + culision_main->height) <= (temp->y + temp->height)){
							...
							( culision_main->y + culision_main->height + tvy ) >= (temp->y + tvy2) )
						{
							...
							if( ( culision_main->x - tvx ) >= (temp->x + temp->width - tvx2 ) )
							{
								...
								//culision_main->setxy(temp->x + temp->width, culision_main->y);
							...
							else if ( ( culision_main->x + culision_main->width - tvx ) <= (temp->x - tvx2 ))
							{
								...
								//culision_main->setxy(temp->x - culision_main->width, culision_main->y);
							}
							else
							{
								double tt;
								tt = atan2( (double)(culision_main->y - temp->y) , (double)(culision_main->x - temp->x) );
								tt = (int)(tt / 3.14 * 180);
								if( tt <= 180 && tt >= 135 || tt >= -180 && tt <= -135 ){
									//culision_main->x = temp->x - culision_main->width;
									culision_main->crash_obj_move_right(temp);
								}
								if( tt <= 45 && tt >= -45){
									//culision_main->x = temp->x + temp->width;
									culision_main->crash_obj_move_left(temp);
								}							
							}
						}
...
void	C_physics::boom_process()
{

}

void C_physics::fixed_obj_draw()
{
	C_obj* temp = fixed_first;
...
void C_physics::fixed_obj_remove()
{
	C_obj* temp = fixed_first, *curr;
	...
	fixed_obj_list = fixed_first = NULL;
	f_list_index = 0;

	movable_obj_list = movable_first = NULL;
	m_list_index = 0;
}